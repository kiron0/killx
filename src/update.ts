...
const REGISTRY_URL = "https://registry.npmjs.org/killx/latest";
...
export async function checkUpdateManually(
  ...
  return {
    ...
    releaseUrl: "https://github.com/kiron0/killx/releases/latest",
    updateAvailable,
  };
}
...
export function installUpdate(version: string): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(command, ["install", "--global", `killx@${version}`], {
      stdio: "inherit",
      windowsHide: true,
    });
...
function updateCacheDirectory(): string {
  ...
  if (configured) return join(configured, "killx");
  if (process.platform === "win32" && process.env.LOCALAPPDATA) {
    return join(process.env.LOCALAPPDATA, "killx");
  }
  return join(homedir(), ".cache", "killx");
}
